...
Adds the **murf** speech provider to your OpenClaw workspace. Every outbound message (or inbound, depending on your `auto` setting) is synthesized into natural-sounding audio using Murf's Falcon model.
...
| model | `FALCON` | `FALCON` (only model supported) |
...
| format | `MP3` | `MP3`, `WAV`, `OGG`, `FLAC` (voice-note targets always use MP3) |
